
//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

using Luban;
using Luban.SimpleJSON;


namespace cfg
{
/// <summary>
/// Task: TaskPlayCue
/// </summary>
public sealed partial class TaskPlayCue : AbilityTaskBase
{
    public TaskPlayCue(JSONNode _buf)  : base(_buf) 
    {
        { if(!_buf["Param"].IsObject) { throw new SerializationException(); }  Param = global::cfg.XParamCue.DeserializeXParamCue(_buf["Param"]);  }
    }

    public static TaskPlayCue DeserializeTaskPlayCue(JSONNode _buf)
    {
        return new TaskPlayCue(_buf);
    }

    /// <summary>
    /// 参数
    /// </summary>
    public readonly XParamCue Param;
   
    public const int __ID__ = -1232667398;
    public override int GetTypeId() => __ID__;

    public override void ResolveRef(Tables tables)
    {
        base.ResolveRef(tables);
        Param?.ResolveRef(tables);
    }

    public override string ToString()
    {
        return "{ "
        + "Param:" + Param + ","
        + "}";
    }
}
}

